...
import { Admin, Anonymous, ExpiredUser, LoggedUser, Moderator } from "./roles";
...
function setup_authorization_stores(access_token) {
	...
	switch (role) {
		...
		case "user":
			...
			break;
		case "moderator":
			role_store.set(new Moderator());
			break;
		case "anonymous":
			role_store.set(new Anonymous());
			break;
		case "expired":
			role_store.set(new ExpiredUser());
			break;
	}
	user_uuid.set(uuid);
}
...
class CustomEndpointManager implements AuthenticationManager {
	...
	login(login: string, password: string): Promise<boolean> {
		return (async () => {
			...
			if (response.ok) {
				const { message, access_token, subscription_due } =
					await response.json();
				if (access_token) {
					subscription_due_date.set(subscription_due);
